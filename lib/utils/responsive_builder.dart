...
class ResponsiveBuilder extends StatelessWidget {
  ...
  static bool isTablet(BuildContext context) => MediaQuery.of(context).size.width >= 641 && MediaQuery.of(context).size.width <= 1007;
  ...
  static bool isTabletButWeb(BuildContext context) => isTablet(context) && kIsWeb;
    ...
    return LayoutBuilder(
      builder: (_, size) {
        if (size.maxWidth >= 1008) {
          return web;
        } else if (size.maxWidth >= 641 && size.maxWidth <= 1007) {
          return tablet;
        } else {
          return mobile;
        }
